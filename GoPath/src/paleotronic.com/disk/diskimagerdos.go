...
var RDOSTypeMap = map[RDOSFileType][2]string{
	...
	FileType_RDOS_AppleSoft: [2]string{"APL", "Applesoft Basic Program"},
...
func (fd *RDOSFileDescriptor) Name() string {
	...
	str = strings.TrimRight(str, " ") + "." + fd.Type().Ext()
...
func (dsk *DSKWrapper) RDOSGetCatalog(pattern string) ([]*RDOSFileDescriptor, error) {
	...
	for i := 0; i < RDOS_CATALOG_LENGTH*RDOS_ENTRY_LENGTH; i++ {
		...
		if !entry.IsDeleted() && rx.MatchString(entry.Name()) {
...
func (dsk *DSKWrapper) RDOSReadFile(file *RDOSFileDescriptor) (int, []byte, error) {

	data, err := dsk.RDOSReadFileRaw(file)

	if err != nil {
		return 0x801, []byte(nil), err
	}

	if file.Type().Ext() == "APP" {
		return file.LoadAddress(), ApplesoftDetoks(data), nil
	} else if file.Type().Ext() == "TXT" {
		return 0x00, StripText(data), nil
	}

	return file.LoadAddress(), data, nil

}

func (dsk *DSKWrapper) RDOSReadFileRaw(file *RDOSFileDescriptor) ([]byte, error) {
