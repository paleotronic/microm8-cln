...
build() {
	...
	#rm $RESOURCEFILES
	./assetbuild.sh
	GO111MODULE=off $GO build -o $EXENAME .
	#git checkout $RESOURCEFILES
}
...
# 	cd ../../../..
     cd ..
	 GOOS=darwin GOARCH=amd64 $XGO --image ktereyp/xgo:1.16.2 --targets="darwin-10.11/amd64" --deps="http://localhost:8080/portaudio19.tar" octalyzer
	#git checkout $RESOURCEFILES
}
