...
func ApplesoftTokenize(lines []string) []byte {
	...
	for _, l := range lines {
		l = strings.Trim(l, " \r\n\t")
		if strings.HasPrefix(l, ":") && len(joinedLines) > 0 {
			joinedLines[len(joinedLines)-1] = joinedLines[len(joinedLines)-1] + l
		} else {
	...
	for _, l := range joinedLines {
		...
		if len(tmp) == 1 {
			ln = 0
			rest = ""
			log.Printf("failed to split: %s", l)
		} else {
