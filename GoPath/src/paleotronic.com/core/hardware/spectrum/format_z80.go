...
func (data SnapshotData) decodeZ80_v1() (*Z80, error) {
...

	{
		...
		if len(mem) != 48*1024 {
			return nil, errors.New("invalid Z80 snapshot - not 48k")
		}
...
func (data SnapshotData) decodeZ80_v2() (*Z80, error) {
	...
	if len(data) < _Z80_V2_HEADER_SIZE {
		return nil, errors.New("invalid Z80 snapshot - not enough data for v2 header")
	}

	extendedHeaderLength := uint16(data[30]) | (uint16(data[31]) << 8)
	if extendedHeaderLength != 23 {
		return nil, errors.New("invalid Z80 snapshot - bad extended header length")
	}
	...
	if s.is128K {
		s.pageState = data[35]
		log.Printf(
			"page register value is %.2x (bank %d, rom %d, screen %d)",
			s.pageState,
			s.pageState&7,
			(s.pageState>>4)&1,
			(s.pageState>>3)&1,
...
func (data SnapshotData) decodeZ80_v3() (*Z80, error) {
	...
	if len(data) < _Z80_V3X_HEADER_SIZE {
		return nil, errors.New("invalid Z80 snapshot - not enough data for v3 header")
	}

	extendedHeaderLength := uint16(data[30]) | (uint16(data[31]) << 8)
	if !((extendedHeaderLength == 54) || (extendedHeaderLength == 55)) {
		return nil, errors.New("invalid Z80 snapshot incorrect extended header length")
	}
...
func z80_loadMemBlocks(s *Z80, data []byte) error {
	...
	if s.is128K {
		s.mem = make([]byte, 128*1024)
	} else {
		s.mem = make([]byte, 48*1024)
	}
	...
	for i+3 <= len(data) {
		...
		if !(i+length <= len(data)) {
			return errors.New("invalid Z80 snapshot - bad length in loadmemblocks")
		}

		if !compressed {
			pages[page] = data[i:(i + length)]
		} else {
			decompressedBlock := unpack_z80page(data[i:(i + length)])
			var ok bool
			for _, v := range decompressedBlock {
				if v != 0 {
					ok = true
				}
			}
			if !ok {
				log.Printf("Z80 RAM Page %d appears empty!", page)
			}
	...
	if i != len(data) {
		return errors.New("invalid Z80 snapshot - length mismatch")
	}

	if !s.is128K && len(pages) != 3 {
		return errors.New("invalid Z80 snapshot - expected 3 pages")
	}

	if s.is128K && len(pages) != 8 {
		return errors.New("invalid Z80 snapshot - expected 8 pages")
	}
		...
		if s.is128K {
			...
			*/
			...
			case 3, 4, 5, 6, 7, 8, 9, 10:
				log.Printf("Found page %d -> bank %d", page, page-3)
				addr = int(page-3) * 0x4000
				length = 0x4000
			default:
				return errors.New("invalid Z80 snapshot - unexpected 128K page number")
			}

			if len(pageData) != length {
				return errors.New("invalid Z80 snapshot - 128K page data length mismatch")
			}

			for i := 0; i < length; i++ {
				s.mem[addr+i] = pageData[i]
			}

			f, _ := os.Create(fmt.Sprintf("bank_%.4x.dat", addr))
			f.Write(pageData)
			f.Close()
			...
			case 5:
				...
				length = 0x4000
			default:
				return errors.New("invalid Z80 snapshot - unexpected 48K page number")
			}

			if len(pageData) != length {
				return errors.New("invalid Z80 snapshot - 48K page data length mismatch")
			}
...
func (s *Z80) Memory() []byte {
	return s.mem
}

func (s *Z80) Is128K() bool {
	return s.is128K
}

func (s *Z80) PageState() byte {
	return s.pageState
}
