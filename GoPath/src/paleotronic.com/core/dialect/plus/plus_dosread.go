...
type PlusDOSREAD struct {
	dialect.CoreFunction
}

func (this *PlusDOSREAD) FunctionExecute(params *types.TokenList) error {
	...
	if !this.Query {
		...
		e := files.DOSREAD(p, f, 0)
		if e != nil {
			this.Interpreter.PutStr(e.Error() + "\r\n")
		} else {
			this.Interpreter.SetInChannel(filename)
		}
...
func (this *PlusDOSREAD) Syntax() string {
...
func (this *PlusDOSREAD) FunctionParams() []types.TokenType {
...
func NewPlusDOSREAD(a int, b int, params types.TokenList) *PlusDOSREAD {
	this := &PlusDOSREAD{}
