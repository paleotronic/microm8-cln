...
type StandardCommandGR struct {
	dialect.Command
}

func (this *StandardCommandGR) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	if mr, ok := caller.GetMemoryMap().InterpreterMappableAtAddress(caller.GetMemIndex(), 0xc000); ok {
		...
		apple2helpers.LOGRClear(caller, 0, 40)
...
func (this *StandardCommandGR) Syntax() string {
