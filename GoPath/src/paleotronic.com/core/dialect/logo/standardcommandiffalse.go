...
type StandardCommandIFFALSE struct {
	dialect.Command
	Split bool
}

func (this *StandardCommandIFFALSE) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	if rr == nil {
		rr = types.NewToken(types.NUMBER, "1")
	}
...
func (this *StandardCommandIFFALSE) Syntax() string {
