...
type StandardCommandRL struct {
	dialect.Command
}

func (this *StandardCommandRL) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	apple2helpers.VECTOR(caller).GetTurtle( this.Command.D.(*DialectLogo).Driver.GetTurtle() ).RollLeft(float32(tt.AsFloat()))
...
func (this *StandardCommandRL) Syntax() string {
