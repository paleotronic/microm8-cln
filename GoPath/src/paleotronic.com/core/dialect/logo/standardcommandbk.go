...
type StandardCommandBK struct {
	dialect.Command
}

func (this *StandardCommandBK) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	apple2helpers.VECTOR(caller).GetTurtle(this.Command.D.(*DialectLogo).Driver.GetTurtle()).Backward(float32(tt.AsFloat()))
...
func (this *StandardCommandBK) Syntax() string {
