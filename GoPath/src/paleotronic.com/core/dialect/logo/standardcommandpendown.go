...
type StandardCommandPENDOWN struct {
	dialect.Command
}

func (this *StandardCommandPENDOWN) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	apple2helpers.VECTOR(caller).GetTurtle( this.Command.D.(*DialectLogo).Driver.GetTurtle() ).SetPenDown()
...
func (this *StandardCommandPENDOWN) Syntax() string {
