...
type StandardCommandTYPESIZE struct {
	dialect.Command
	solid bool
}

func (this *StandardCommandTYPESIZE) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	apple2helpers.VECTOR(caller).GetTurtle(this.Command.D.(*DialectLogo).Driver.GetTurtle()).GlyphSize(
		float32(tt.AsFloat()),
	)
...
func (this *StandardCommandTYPESIZE) Syntax() string {
