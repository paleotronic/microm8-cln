...
type StandardCommandDN struct {
	dialect.Command
}

func (this *StandardCommandDN) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	apple2helpers.VECTOR(caller).GetTurtle( this.Command.D.(*DialectLogo).Driver.GetTurtle() ).Down(float32(tt.AsFloat()))
...
func (this *StandardCommandDN) Syntax() string {
