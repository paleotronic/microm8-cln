...
type StandardCommandRT struct {
	dialect.Command
}

func (this *StandardCommandRT) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	apple2helpers.VECTOR(caller).GetTurtle( this.Command.D.(*DialectLogo).Driver.GetTurtle() ).Right(float32(tt.AsFloat()))
...
func (this *StandardCommandRT) Syntax() string {
