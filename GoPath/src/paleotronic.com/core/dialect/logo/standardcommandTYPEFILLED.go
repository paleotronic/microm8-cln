...
type StandardCommandTYPEFILLED struct {
	dialect.Command
	solid bool
}

func (this *StandardCommandTYPEFILLED) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	apple2helpers.VECTOR(caller).GetTurtle(this.Command.D.(*DialectLogo).Driver.GetTurtle()).GlyphFilled(
		tt.AsInteger(),
	)
...
func (this *StandardCommandTYPEFILLED) Syntax() string {
