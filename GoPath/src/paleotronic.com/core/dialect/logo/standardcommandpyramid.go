...
type StandardCommandPYRAMID struct {
	dialect.Command
	solid bool
}

func (this *StandardCommandPYRAMID) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	apple2helpers.VECTOR(caller).GetTurtle(this.Command.D.(*DialectLogo).Driver.GetTurtle()).Pyramid(
		float32(tt.AsFloat()),
		float32(tt.AsFloat()),
		this.solid,
	)
...
func (this *StandardCommandPYRAMID) Syntax() string {
