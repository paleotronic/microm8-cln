...
type StandardCommandCIRCLE struct {
	dialect.Command
	solid bool
}

func (this *StandardCommandCIRCLE) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	apple2helpers.VECTOR(caller).GetTurtle(this.Command.D.(*DialectLogo).Driver.GetTurtle()).Circle(
		float32(tt.AsFloat()),
		this.solid,
	)
...
func (this *StandardCommandCIRCLE) Syntax() string {
