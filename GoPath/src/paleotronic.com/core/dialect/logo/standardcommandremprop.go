...
type StandardCommandREMPROP struct {
	dialect.Command
}

func (this *StandardCommandREMPROP) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	if foundIdx == -1 {
		// new property - do nothing
	} else {
		// existing property
		lobj.List.Remove(foundIdx)
		lobj.List.Remove(foundIdx)
	}

	lobj.IsPropList = true
...
func (this *StandardCommandREMPROP) Syntax() string {
