...
type StandardCommandUP struct {
	dialect.Command
}

func (this *StandardCommandUP) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	apple2helpers.VECTOR(caller).GetTurtle( this.Command.D.(*DialectLogo).Driver.GetTurtle() ).Up(float32(tt.AsFloat()))
...
func (this *StandardCommandUP) Syntax() string {
