...
type StandardCommandTYPEFACE struct {
	dialect.Command
	solid bool
}

func (this *StandardCommandTYPEFACE) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	apple2helpers.VECTOR(caller).GetTurtle(this.Command.D.(*DialectLogo).Driver.GetTurtle()).GlyphFace(
		tt.AsInteger(),
	)
...
func (this *StandardCommandTYPEFACE) Syntax() string {
