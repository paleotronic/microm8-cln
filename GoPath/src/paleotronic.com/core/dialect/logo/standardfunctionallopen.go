...
type StandardFunctionALLOPEN struct {
	dialect.CoreFunction
}

func (this *StandardFunctionALLOPEN) FunctionParams() []types.TokenType {
...
func NewStandardFunctionALLOPEN(a int, b int, params types.TokenList) *StandardFunctionALLOPEN {
	this := &StandardFunctionALLOPEN{}
...
func (this *StandardFunctionALLOPEN) FunctionExecute(params *types.TokenList) error {
	...
	// request input here
	files := files.DOSBUFFERS()

	for _, f := range files {
		thing.List.Push(
			types.NewToken( types.WORD, f ),
		)
	}
...
func (this *StandardFunctionALLOPEN) Syntax() string {

	/* vars */
	var result string

	result = "ALLOPEN word list"
...
func (this *StandardFunctionALLOPEN) GetCRTLine(caller interfaces.Interpretable) string {
