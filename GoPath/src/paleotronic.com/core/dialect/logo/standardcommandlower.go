...
type StandardCommandLOWER struct {
	dialect.Command
}

func (this *StandardCommandLOWER) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	apple2helpers.VECTOR(caller).GetTurtle( this.Command.D.(*DialectLogo).Driver.GetTurtle() ).Lower(float32(tt.AsFloat()))
...
func (this *StandardCommandLOWER) Syntax() string {
