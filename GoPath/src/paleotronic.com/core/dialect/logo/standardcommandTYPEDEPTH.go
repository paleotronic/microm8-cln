...
type StandardCommandTYPEDEPTH struct {
	dialect.Command
	solid bool
}

func (this *StandardCommandTYPEDEPTH) Execute(env *interfaces.Producable, caller interfaces.Interpretable, tokens types.TokenList, Scope *types.Algorithm, LPC types.CodeRef) (int, error) {
	...
	apple2helpers.VECTOR(caller).GetTurtle(this.Command.D.(*DialectLogo).Driver.GetTurtle()).GlyphDepth(
		float32(tt.AsFloat()),
	)
...
func (this *StandardCommandTYPEDEPTH) Syntax() string {
